//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

#import <SystemStatusServer/NSXPCListenerDelegate-Protocol.h>

@class NSString, NSXPCListener;
@protocol STStatusDomainServerHandle;

@interface STStatusDomainXPCClientListener : NSObject <NSXPCListenerDelegate>
{
    id <STStatusDomainServerHandle> _serverHandle;	// 8 = 0x8
    NSXPCListener *_xpcListener;	// 16 = 0x10
}

- (void).cxx_destruct;	// IMP=0x0000000000012ede
@property(readonly, nonatomic) NSXPCListener *xpcListener; // @synthesize xpcListener=_xpcListener;
@property(readonly, nonatomic) __weak id <STStatusDomainServerHandle> serverHandle; // @synthesize serverHandle=_serverHandle;
- (_Bool)listener:(id)arg1 shouldAcceptNewConnection:(id)arg2;	// IMP=0x0000000000012e33
- (void)dealloc;	// IMP=0x0000000000012dc8
- (id)initWithServerHandle:(id)arg1;	// IMP=0x0000000000012d09

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

