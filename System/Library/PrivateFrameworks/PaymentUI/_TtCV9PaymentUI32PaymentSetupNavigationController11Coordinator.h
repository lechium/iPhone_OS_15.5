//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

#import <PaymentUI/PKPaymentSetupDelegate-Protocol.h>

@class MISSING_TYPE;

__attribute__((visibility("hidden")))
@interface _TtCV9PaymentUI32PaymentSetupNavigationController11Coordinator : NSObject <PKPaymentSetupDelegate>
{
    MISSING_TYPE *parent;	// 8 = 0x8
    MISSING_TYPE *setupFinishedAction;	// 32 = 0x20
}

- (void).cxx_destruct;	// IMP=0x0000000000097b00
- (id)init;	// IMP=0x0000000000097aa0
- (void)paymentSetupDidFinish:(id)arg1;	// IMP=0x0000000000097a40

@end

