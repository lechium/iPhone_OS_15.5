//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

#import <SpringBoard/BBObserverDelegate-Protocol.h>

@class BBObserver, DNDEventBehaviorResolutionService, NSString;

@interface SBBulletinSpokenObserverGateway : NSObject <BBObserverDelegate>
{
    BBObserver *_bbObserver;	// 8 = 0x8
    DNDEventBehaviorResolutionService *_dndEventBehaviorResolutionService;	// 16 = 0x10
}

+ (id)sharedInstance;	// IMP=0x00000000001ac0eb
- (void).cxx_destruct;	// IMP=0x00000000001ac170

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

