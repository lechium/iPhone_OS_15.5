//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

@class SBAppLayout;

@interface SBIconOverlayVisibilitySwitcherEventResponse
{
    _Bool _visible;	// 8 = 0x8
    SBAppLayout *_appLayout;	// 16 = 0x10
}

- (void).cxx_destruct;	// IMP=0x00000000004efe24
@property(readonly, nonatomic, getter=isVisible) _Bool visible; // @synthesize visible=_visible;
@property(readonly, nonatomic) SBAppLayout *appLayout; // @synthesize appLayout=_appLayout;
- (id)descriptionBuilderWithMultilinePrefix:(id)arg1;	// IMP=0x00000000004efd42
- (id)initWithAppLayout:(id)arg1 visible:(_Bool)arg2;	// IMP=0x00000000004efcc2
- (long long)type;	// IMP=0x00000000004efcb7

@end

