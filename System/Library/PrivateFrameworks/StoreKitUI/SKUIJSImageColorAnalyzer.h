//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <ITMLKit/IKJSObject.h>

#import <StoreKitUI/SKUIArtworkRequestDelegate-Protocol.h>
#import <StoreKitUI/SKUIJSImageColorAnalyzer-Protocol.h>

@class NSMutableDictionary, NSOperationQueue, NSString, SKUIClientContext, SKUIResourceLoader;

__attribute__((visibility("hidden")))
@interface SKUIJSImageColorAnalyzer : IKJSObject <SKUIArtworkRequestDelegate, SKUIJSImageColorAnalyzer>
{
    SKUIClientContext *_clientContext;	// 8 = 0x8
    NSMutableDictionary *_artworkRequestIDsToManagedCallbacks;	// 16 = 0x10
    NSOperationQueue *_operationQueue;	// 24 = 0x18
    SKUIResourceLoader *_resourceLoader;	// 32 = 0x20
}

- (void).cxx_destruct;	// IMP=0x000000000023ed95
- (void)artworkRequest:(id)arg1 didLoadImage:(id)arg2;	// IMP=0x000000000023e8fa
- (void)analyzeImageAtUrl:(id)arg1:(id)arg2;	// IMP=0x000000000023e655
- (void)dealloc;	// IMP=0x000000000023e60c
- (id)initWithAppContext:(id)arg1 clientContext:(id)arg2;	// IMP=0x000000000023e4f5

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

