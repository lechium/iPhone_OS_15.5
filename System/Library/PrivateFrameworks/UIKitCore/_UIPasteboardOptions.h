//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

@class NSDate;

@interface _UIPasteboardOptions : NSObject
{
    _Bool _pinned;	// 8 = 0x8
    _Bool _localOnly;	// 9 = 0x9
    NSDate *_expirationDate;	// 16 = 0x10
}

+ (id)optionsWithDictionary:(id)arg1;	// IMP=0x0000000000bd628a
- (void).cxx_destruct;	// IMP=0x0000000000bd6506
@property(retain, nonatomic) NSDate *expirationDate; // @synthesize expirationDate=_expirationDate;
@property(nonatomic, getter=isLocalOnly) _Bool localOnly; // @synthesize localOnly=_localOnly;
@property(nonatomic, getter=isPinned) _Bool pinned; // @synthesize pinned=_pinned;
- (id)initWithOptionsDictionary:(id)arg1;	// IMP=0x0000000000bd62d3

@end

