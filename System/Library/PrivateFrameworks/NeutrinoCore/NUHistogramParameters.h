//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

#import <NeutrinoCore/NSCopying-Protocol.h>

@interface NUHistogramParameters : NSObject <NSCopying>
{
    CDStruct_68f7c993 _colorMatrix;	// 8 = 0x8
}

@property CDStruct_68f7c993 colorMatrix; // @synthesize colorMatrix=_colorMatrix;
- (id)description;	// IMP=0x0000000000077e0c
- (id)copyWithZone:(struct _NSZone *)arg1;	// IMP=0x0000000000077dc2
- (id)init;	// IMP=0x0000000000077d67

@end

