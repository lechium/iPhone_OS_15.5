//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

@protocol NTKComplicationPickerListProvider;

@interface NTKComplicationPickerBaseViewConfiguration : NSObject
{
    id <NTKComplicationPickerListProvider> _listProvider;	// 8 = 0x8
}

- (void).cxx_destruct;	// IMP=0x00000000000b3be2
@property(readonly, nonatomic) id <NTKComplicationPickerListProvider> listProvider; // @synthesize listProvider=_listProvider;
- (id)initWithListProvider:(id)arg1;	// IMP=0x00000000000b3b6d

@end

