//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

#import <WorkflowUI/WFWorkflowSettingsViewControllerDelegate-Protocol.h>

@class NSString;

@interface WFSimplePresentedWorkflowSettingsViewControllerDelegate : NSObject <WFWorkflowSettingsViewControllerDelegate>
{
}

- (_Bool)settingsViewControllerWantsToKnowIfNewShortcutFlow:(id)arg1;	// IMP=0x000000000015d34a
- (void)settingsViewControllerWantsWorkflowReload:(id)arg1;	// IMP=0x000000000015d344

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

