//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

@class SFRankingFeedback;

@interface FCArticleSearchOperationFeedbackResult : NSObject
{
    SFRankingFeedback *_resultRankingFeedback;	// 8 = 0x8
}

- (void).cxx_destruct;	// IMP=0x0000000000217861
@property(readonly, copy, nonatomic) SFRankingFeedback *resultRankingFeedback; // @synthesize resultRankingFeedback=_resultRankingFeedback;
- (id)initWithSearchOperationRankingFeedBack:(id)arg1;	// IMP=0x00000000002177e4

@end

