//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <UIKit/UITableViewController.h>

@class MCDPlayableContentPlaybackManager;

@interface MCDPlayableContentQueueTableViewController : UITableViewController
{
    MCDPlayableContentPlaybackManager *_contentManager;	// 8 = 0x8
}

- (void).cxx_destruct;	// IMP=0x000000000001fb32
@property(retain, nonatomic) MCDPlayableContentPlaybackManager *contentManager; // @synthesize contentManager=_contentManager;
- (void)viewDidLoad;	// IMP=0x000000000001f9fd
- (id)initWithContentManager:(id)arg1;	// IMP=0x000000000001f98f

@end

