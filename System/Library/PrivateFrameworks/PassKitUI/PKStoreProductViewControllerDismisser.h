//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

#import <PassKitUI/SKStoreProductViewControllerDelegate-Protocol.h>

@class NSString, SKStoreProductViewController;

@interface PKStoreProductViewControllerDismisser : NSObject <SKStoreProductViewControllerDelegate>
{
    SKStoreProductViewController *_productViewController;	// 8 = 0x8
}

- (void).cxx_destruct;	// IMP=0x00000000000a46ed
- (void)productViewControllerDidFinish:(id)arg1;	// IMP=0x00000000000a4674
- (void)attachToProductViewController:(id)arg1;	// IMP=0x00000000000a45dd
- (void)dealloc;	// IMP=0x00000000000a4581

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

