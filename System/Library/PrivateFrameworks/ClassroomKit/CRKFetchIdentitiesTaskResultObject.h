//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <DeviceManagement/CATTaskResultObject.h>

@class NSDictionary;

@interface CRKFetchIdentitiesTaskResultObject : CATTaskResultObject
{
    NSDictionary *_courseIdentityInfosByGroupIdentifier;	// 8 = 0x8
}

+ (_Bool)supportsSecureCoding;	// IMP=0x00000000000b462f
- (void).cxx_destruct;	// IMP=0x00000000000b4880
@property(copy, nonatomic) NSDictionary *courseIdentityInfosByGroupIdentifier; // @synthesize courseIdentityInfosByGroupIdentifier=_courseIdentityInfosByGroupIdentifier;
- (id)initWithCoder:(id)arg1;	// IMP=0x00000000000b46b0
- (void)encodeWithCoder:(id)arg1;	// IMP=0x00000000000b4637

@end

