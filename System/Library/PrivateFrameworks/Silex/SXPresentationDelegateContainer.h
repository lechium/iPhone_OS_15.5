//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

#import <Silex/SXPresentationDelegateContainer-Protocol.h>

@class NSString;
@protocol SXPresentationDelegate;

@interface SXPresentationDelegateContainer : NSObject <SXPresentationDelegateContainer>
{
    id <SXPresentationDelegate> _presentationDelegate;	// 8 = 0x8
}

- (void).cxx_destruct;	// IMP=0x00000000001304bf
@property(readonly, nonatomic) __weak id <SXPresentationDelegate> presentationDelegate; // @synthesize presentationDelegate=_presentationDelegate;
- (void)registerPresentationDelegate:(id)arg1;	// IMP=0x0000000000130498

// Remaining properties
@property(readonly, copy) NSString *debugDescription;
@property(readonly, copy) NSString *description;
@property(readonly) unsigned long long hash;
@property(readonly) Class superclass;

@end

