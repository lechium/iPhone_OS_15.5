//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

@class NSOperationQueue;

@interface TSKDocumentRootICloudObserver : NSObject
{
    long long _identifier;	// 8 = 0x8
    NSOperationQueue *_operationQueue;	// 16 = 0x10
    CDUnknownBlockType _block;	// 24 = 0x18
    long long _identifer;	// 32 = 0x20
}

@property(readonly, nonatomic) long long identifer; // @synthesize identifer=_identifer;
- (void)invokeWithDocumentRoot:(id)arg1;	// IMP=0x0000000000084256
- (void)dealloc;	// IMP=0x000000000008421b
- (id)initWithBlock:(CDUnknownBlockType)arg1;	// IMP=0x00000000000841af

@end

