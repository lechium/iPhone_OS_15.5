//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

@protocol CHSChronoWidgetServiceServer;

@interface CHSApplicationProcessStateChangeConsumer : NSObject
{
    id <CHSChronoWidgetServiceServer> _server;	// 8 = 0x8
}

- (void).cxx_destruct;	// IMP=0x0000000000007b12
@property(readonly, nonatomic) id <CHSChronoWidgetServiceServer> server; // @synthesize server=_server;
- (void)applicationWithBundleIdentifierEnteredForeground:(id)arg1;	// IMP=0x0000000000007af1
- (id)initWithServer:(id)arg1;	// IMP=0x0000000000007a72
- (id)init;	// IMP=0x00000000000079fd

@end

