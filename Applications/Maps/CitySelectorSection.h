//
//     Generated by classdump-c 4.2.0 (64 bit).
//
//  Copyright (C) 1997-2019 Steve Nygard. Updated in 2022 by Kevin Bradley.
//

#import <objc/NSObject.h>

@class MKPlaceCompactCollectionSizeController, NSString;

__attribute__((visibility("hidden")))
@interface CitySelectorSection : NSObject
{
    NSString *_sectionTitle;	// 8 = 0x8
    MKPlaceCompactCollectionSizeController *_sizeController;	// 16 = 0x10
    long long _kind;	// 24 = 0x18
    CDStruct_9f748a59 _configuration;	// 32 = 0x20
}

- (void).cxx_destruct;	// IMP=0x002000000031f54d
@property(nonatomic) long long kind; // @synthesize kind=_kind;
@property(retain, nonatomic) MKPlaceCompactCollectionSizeController *sizeController; // @synthesize sizeController=_sizeController;
@property(retain, nonatomic) NSString *sectionTitle; // @synthesize sectionTitle=_sectionTitle;
@property(nonatomic) CDStruct_f6ae2b44 configuration; // @synthesize configuration=_configuration;
- (id)layoutSectionForWorldwideUsingTraitsEnvironment:(id)arg1 usingWidth:(double)arg2;	// IMP=0x001000000031f137
- (id)layoutSectionForCompactCollectionsUsingTraitsEnvironment:(id)arg1 usingWidth:(double)arg2;	// IMP=0x001000000031ec5f
- (void)initializeSizeControllerUsingTraitEnvironment:(id)arg1;	// IMP=0x001000000031ebd9
- (void)setupConfiguration;	// IMP=0x001000000031eb4f
- (id)layoutForSectionUsingTraitsEnvironment:(id)arg1 usingWidth:(double)arg2;	// IMP=0x001000000031eacc
- (id)initWithSectionKind:(long long)arg1 usingMaxWidth:(double)arg2 usingTraitEnvironment:(id)arg3;	// IMP=0x001000000031ea48

@end

